...
#include "krr_mirte_pddl/krr_mirte_pddl.hpp"
...
namespace krr_mirte_pddl
{
  ...
  KrrMirtePddl::KrrMirtePddl(const std::string & node_name)
  : rclcpp::Node(node_name)
{
  ...
  step_timer_ = this->create_wall_timer(
    1s, std::bind(&KrrMirtePddl::step, this), step_timer_cb_group_);

}

KrrMirtePddl::~KrrMirtePddl()
{
}
...
void KrrMirtePddl::finish_controlling(){
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<krr_mirte_pddl::KrrMirtePddl>(
    "krr_mirte_pddl_node");
